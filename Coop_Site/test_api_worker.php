...
function handlePdfUpload($mysqli, $worker_id) {
    ...
    try {
        $query = "INSERT INTO reportes_pdf (trabajador_id, titulo, descripcion, nombre_archivo, ruta_archivo, tamano_archivo, fecha_envio) 
                  VALUES (?, ?, ?, ?, ?, ?, NOW())";
        ...
        $file_size = $file['size'];
        $stmt->bind_param('issssi', $worker_id, $titulo, $descripcion, $original_name, $file_path, $file_size);
        ...
        logError("PDF subido exitosamente. Report ID: $report_id");
        
        sendJsonResponse([
            'success' => true,
            'message' => 'Reporte PDF subido exitosamente',
            'data' => [
                'report_id' => $report_id,
