...
// El objeto user tiene 'nombre' no 'name' (viene de user_auth.php)
...
$stats = [
    ...
    'reportes_enviados' => 0, // Alias para compatibilidad
    'ultimo_comprobante' => null,
    'ultimo_reporte' => null // Alias para compatibilidad
];
...
try {
    ...
    // Comprobantes enviados este mes (desde base de datos)
    require_once __DIR__ . '/includes/comprobantes_db.php';
    $comprobantes_mes = getComprobantes();
    $mes_actual = date('Y-m');
    $comprobantes_este_mes = 0;
    $ultimo_comprobante_fecha = null;
    
    foreach ($comprobantes_mes as $comp) {
        if ($comp['trabajador_id'] == $worker_id) {
            $fecha_envio = $comp['fecha_envio'];
            if ($fecha_envio) {
                $fecha = new DateTime($fecha_envio);
                if ($fecha->format('Y-m') === $mes_actual) {
                    $comprobantes_este_mes++;
                }
                if (!$ultimo_comprobante_fecha || $fecha_envio > $ultimo_comprobante_fecha) {
                    $ultimo_comprobante_fecha = $fecha_envio;
                }
            }
        }
    }
    
    $stats['comprobantes_enviados'] = $comprobantes_este_mes;
    $stats['reportes_enviados'] = $comprobantes_este_mes; // Alias para compatibilidad
    $stats['ultimo_comprobante'] = $ultimo_comprobante_fecha;
    $stats['ultimo_reporte'] = $ultimo_comprobante_fecha; // Alias para compatibilidad
...
$current_page = isset($_GET['page']) ? limpiarDato($_GET['page']) : 'dashboard';
$valid_pages = ['dashboard', 'historial', 'pagos', 'estadisticas'];
...
if (isset($_GET['success'])) {
    switch ($_GET['success']) {
        case 'time_saved':
            ...
            break;
            case 'pdf_uploaded':
            $success_message = 'Comprobante de pago subido correctamente';
            break;
    }
}
...
<body class="bg-gray-50" x-data="workerApp()" x-init="init(); checkMissingInfo()">
...
<div class="flex h-screen overflow-hidden" x-data="{ sidebarOpen: false }">
    ...
    <aside class="fixed inset-y-0 left-0 z-30 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0" 
        ...
        <nav class="flex-1 px-4 py-6 space-y-2 custom-scroll overflow-y-auto h-full">
            <a href="dashboard.php" class="flex items-center px-3 py-2 rounded-lg <?php echo $current_page === 'dashboard' ? 'bg-green-100 text-green-700 font-medium' : 'text-gray-700 hover:bg-gray-100'; ?> transition-colors">
            ...
            <a href="dashboard.php?page=historial" class="flex items-center px-3 py-2 rounded-lg <?php echo $current_page === 'historial' ? 'bg-green-100 text-green-700 font-medium' : 'text-gray-700 hover:bg-gray-100'; ?> transition-colors">
                <i class="ti ti-history mr-3 text-lg"></i>
                Mi Historial
            </a>
            
            <a href="dashboard.php?page=pagos" class="flex items-center px-3 py-2 rounded-lg <?php echo $current_page === 'pagos' ? 'bg-green-100 text-green-700 font-medium' : 'text-gray-700 hover:bg-gray-100'; ?> transition-colors">
                <i class="ti ti-wallet mr-3 text-lg"></i>
                Pagos Pendientes
            </a>
            
            <a href="dashboard.php?page=estadisticas" class="flex items-center px-3 py-2 rounded-lg <?php echo $current_page === 'estadisticas' ? 'bg-green-100 text-green-700 font-medium' : 'text-gray-700 hover:bg-gray-100'; ?> transition-colors">
                <i class="ti ti-chart-bar mr-3 text-lg"></i>
                Estadísticas
            </a>
        </nav>
    </aside>
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <main class="flex-1 overflow-y-auto p-6 custom-scroll">
            ...
            <?php if ($current_page === 'dashboard'): ?>
                <!-- Dashboard Principal -->
                <?php include 'worker_pages/dashboard_main.php'; ?>
            <?php elseif ($current_page === 'historial'): ?>
                <!-- Historial -->
                <?php include 'worker_pages/historial.php'; ?>
            <?php elseif ($current_page === 'pagos'): ?>
                <!-- Pagos Pendientes -->
                <?php include 'worker_pages/pagos_pendientes.php'; ?>
                                <?php else: ?>
    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-lg bg-white">
        <div class="mt-3">
            ...
            <form @submit.prevent="submitPdfReport()" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Título del comprobante *</label>
                    <input type="text" 
                           x-model="pdfReport.titulo" 
                           placeholder="Comprobante de pago, recibo, etc."
                ...
                <div>
                    ...
                    <textarea x-model="pdfReport.descripcion" 
                              rows="3" 
                              placeholder="Breve descripción del contenido del comprobante..."
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div class="flex justify-end space-x-3 mt-6">
                    ...
                    <button type="submit" :disabled="submitting || !selectedFile"
                        ...
                        <span x-text="submitting ? 'Subiendo...' : 'Subir Comprobante'"></span>
                    </button>
                </div>
            </form>
        </div>
...
function workerApp() {
    return {
        ...
        async checkMissingInfo() {
            try {
                const response = await fetch('api_worker.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        'X-Requested-With': 'XMLHttpRequest'
                    },
                    body: 'action=check_profile_info'
                });
                
                if (response.ok) {
                    const data = await response.json();
                    if (data.success && data.missing_fields && data.missing_fields.length > 0) {
                        // Mostrar modal para completar información
                        this.showMissingInfoModal(data.missing_fields);
                    }
                }
            } catch (error) {
        ...
        showMissingInfoModal(missingFields) {
            const fieldsText = missingFields.map(f => {
                if (f === 'telefono') return 'Número de Teléfono';
                if (f === 'ci') return 'Cédula de Identidad';
                return f;
            }).join(' y ');
            
            if (confirm(`Por favor completa tu información de perfil.\n\nFaltan los siguientes datos: ${fieldsText}\n\n¿Deseas completarlos ahora?`)) {
                window.location.href = 'worker_profile.php?page=configuracion';
            }
        },
        ...
        cancelTimer() {
            if (this.currentTime > 0) {
                if (confirm('¿Estás seguro de que deseas cancelar el timer? Se perderá todo el tiempo registrado.')) {
                    this.resetTimer();
                    this.showToast('Timer cancelado', 'warning');
                }
            }
        },
        ...
        async submitPdfReport() {
            ...
            try {
                ...
                if (data.success) {
                    this.showToast(data.message || 'Comprobante subido exitosamente', 'success');
                    this.closePdfModal();
                    setTimeout(() => window.location.reload(), 1000);
                } else {
                    throw new Error(data.error || data.message || 'Error desconocido al subir comprobante');
                }
